...
class userController extends mainModel
{
    ...
    public function actualizarUsuarioControlador()
    {
        ...
        # Verificando usuario #
        $sql = $this->conectar()->prepare("SELECT u.idUsuario AS usuario_id, 
        ...
        $sql = $this->conectar()->prepare("SELECT * FROM usuario WHERE usuario_usuario = :usuario AND idUsuario = :id");
        $sql->bindParam(':usuario', $admin_usuario);
        $sql->bindParam(':id', $_SESSION['id']);
        $sql->execute();

        if ($sql->rowCount() == 1) {
            $check_admin = $sql->fetch();
            if ($check_admin['usuario_usuario'] != $admin_usuario || !password_verify($admin_clave, $check_admin['password'])) {
                $alerta = [
                    "tipo" => "simple",
                    "titulo" => "Ocurrió un error inesperado",
        ...
        if ($email != "" && $datos['usuario_email'] != $email) {
            if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $sql = $this->conectar()->prepare("SELECT usuario_email FROM usuario WHERE usuario_email = :email");
        ...
        if ($clave1 != "" || $clave2 != "") {
            ...
            if ($clave1 != $clave2) {
                $alerta = [
                    "tipo" => "simple",
                    "titulo" => "Ocurrió un error inesperado",
                    "texto" => "Las CLAVES que ingresó no coinciden, verifique e intente nuevamente",
                    "icono" => "error"
                ];
                return json_encode($alerta);
                exit();
        ...
        $campos = "";
        foreach ($usuario_datos_up as $index => $campo) {
            if ($index > 0)
                $campos .= ", ";
            $campos .= $campo["campo_nombre"] . " = " . $campo["campo_marcador"];
        }

        $sql = $this->conectar()->prepare("UPDATE usuario SET $campos WHERE idUsuario = :ID");
        $sql->bindParam(":ID", $id);

        foreach ($usuario_datos_up as $campo) {
        ...
        if ($sql->execute()) {
            ...
            $alerta = [
                "tipo" => "simple",
                "titulo" => "Ocurrió un error inesperado",
                "texto" => "No hemos podido actualizar el usuario, por favor intente nuevamente",
                "icono" => "error"
            ];
        }
        return json_encode($alerta);
